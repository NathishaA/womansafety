...
const Dashboard = () => {
  ...
  const handleSubmit = () => {
    if (dialogType === 'users') {
      ...
      if (currentItem) {
        axios.put(`http://localhost:8080/api/deliveries/${currentItem.id}`, formData)
          .then(() => {
        ...
        axios.post('http://localhost:8080/api/deliveries/request', formData)
          .then(() => {
  ...
  const handleDelete = (type, id) => {
    if (type === 'users') {
      ...
      axios.delete(`http://localhost:8080/api/deliveries/${id}`)
        .then(() => {
